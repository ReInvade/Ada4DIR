...
class SAM1(nn.Module):
    def __init__(self, n_feat, kernel_size, bias):
        ...
        self.conv1 = conv(n_feat, n_feat//8, kernel_size, bias=bias)
        self.conv2 = conv(n_feat//8, 3, kernel_size, bias=bias)
        self.conv3 = conv(3, n_feat//8, kernel_size, bias=bias)
        self.conv4 = conv(n_feat//8, n_feat, kernel_size, bias=bias)

    def forward(self, x, x_img):
        res = self.conv1(x)
        clean_img = x_img - self.conv2(res)
        x2 = torch.sigmoid(self.conv3(clean_img))
        x1 = self.conv4(res*x2)
        x1 = x1+x
        # x1 = x-x1
        return x1, clean_img
